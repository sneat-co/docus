...
export const appRoutes: Route[] = [
  {
    ...
    pathMatch: 'full',
    loadComponent: () =>
      import('./home/docus-home-page.component').then(
        (m) => m.DocusHomePageComponent,
      ),
    canActivate: [AuthGuard],
    data: { authGuardPipe: () => redirectToLoginIfNotSignedIn },
  },
  {
    // Space-scoped routes host the docus pages, mirroring sneat-app's
    // space/:spaceType/:spaceID mount point.
    path: 'space/:spaceType/:spaceID',
    loadChildren: () =>
      import('./space/docus-space.routes').then((m) => m.docusSpaceRoutes),
  },
  {
